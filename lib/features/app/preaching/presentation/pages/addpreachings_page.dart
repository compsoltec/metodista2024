...
class AddPreachingPage extends StatefulWidget {
  const AddPreachingPage({super.key});

  @override
  State<AddPreachingPage> createState() => _AddPreachingPageState();
}

class _AddPreachingPageState extends State<AddPreachingPage> {
  ...
  Future<void> _savePreaching() async {
    ...
    try {
      final imageUrl =
          await _uploadFileToFirebase(imageFile!.path, 'preachings/images');
      final audioUrl = await _uploadFileToFirebase(
          audioFile!.files.single.path!, 'preachings/audios');
      ...
      sl<PreachingBloc>().add(AddPreaching(preaching));

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Pregação adicionada com sucesso!'),
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  onPressed: isLoading ? null : _savePreaching,
