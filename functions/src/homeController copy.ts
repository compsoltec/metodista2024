...
const addNewHome = async (req: Request, res: Response) => {
    const {aniversariantes, pastoral,programacao} = req.body
    try{
 ...
 const updateHome = async (req: Request, res: Response) => {
    const { body: {aniversariantes, pastoral, programacao }, params: { homeid } } = req
    ...
    try {
      ...
      const homeObject = {
        programacao: programacao || currentData.programacao,
